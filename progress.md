...
- **主链**：连接 Bridge →（可选）流程步骤 → `aiAssert` → 页面上下文取数 / `aiQuery` → 规则引擎 → 执行记录与本地通知。
- **规划方向**：在可信内网场景下补强「开发者取数模式」，允许懂技术的用户复用已登录页面上下文直接执行 fetch / XHR / JS，读取 JSON 或 DOM 后进入规则引擎，减少视觉点击和大模型 token 消耗。
...
- **开发者接口巡检**：例如用户通过 DevTools 找到后台 JSON 接口后，在工作台里配置页面上下文请求、字段映射和规则，定时直接取接口数据；Recorder 主要承担导航和筛选，不作为长期稳定取数的唯一方式。
